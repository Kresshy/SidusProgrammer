...
public class SidusProgram implements Serializable {
	...
	int functions, servos;
	byte[][] data;

	public SidusProgram(int functions, int servos, SidusService service) { // can be set after the
														// version read by the
														// timer.
...
//				data[i][j] = intToByteArray(i * j);
				...
				data[i][j] = intToByteArray(i + j);
//				Log.i(TAG, Byte.toString(data[i][j]));
			}
		}
...
//		out.write(0x00);
		...
		for (byte i = 0; i < functions; i++) {
			...
			byte[] moveToFunc = new byte[] {
					0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x69, i, intToByteArray(CHK_MOVETOFUNC), 0x45
			};
			
			mSidusService.write(moveToFunc);
			
			
//			Log.i(TAG, "CHK_MOVETOFUNC: " + Byte.toString(intToByteArray(CHK_MOVETOFUNC)));
			...
			for (byte j = 0; j < servos; j++) {
				...
				byte[] setServo = new byte[] {
						0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x63, i, j, data[i][j], intToByteArray(CHK_SETSERVO), 0x45
				};
				
				mSidusService.write(setServo);
				
//				Log.i(TAG, "CHK_SETSERVO: " + Byte.toString(intToByteArray(CHK_SETSERVO)));
